      ...
      if (entityObj.isIPv4 || (entityObj.isIPv6 && !entityObj.isPrivateIP)) {
        _lookupEntity(entityObj, options, function(err, result) {
          if (err) {
            ...
            log.debug({ result: result }, 'Printing out the Results');
            next(null);
          }
        });
      } else {
...
function validateOptions(userOptions, cb) {
  ...
  if (
    typeof userOptions.type.value !== 'string' ||
  ...
  request(
    ...
    function(err, response, body) {
      ...
      if (response.statusCode === 404) {
        cb(
          _createJsonErrorPayload('Not Found', null, '404', '2A', 'Not Found', {
            err: err
          })
        );
        return;
      ...
      if (response.statusCode === 400) {
        cb(
          _createJsonErrorPayload('Invalid Search, please check search parameters', null, '400', '2A', 'Bad Request', {
            err: err
          })
        );
        return;
      ...
      if (response.statusCode === 409) {
        cb(
          _createJsonErrorPayload('There was a conflict with your search', null, '409', '2A', 'Conflict', {
            err: err
          })
        );
        return;
      ...
      if (response.statusCode === 503) {
        cb(
          _createJsonErrorPayload('Service is currently unavailable for search results', null, '503', '2A', '', {
            err: err
          })
        );
        return;
